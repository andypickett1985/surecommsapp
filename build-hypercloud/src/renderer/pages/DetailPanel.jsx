...
function DefaultPanel() {
  ...
  async function onCcStatusChange(nextStatus) {
    ...
    try {
      ...
      setState(prev => ({
        callCenter: {
          ...prev.callCenter,
          agent: prev.callCenter?.agent ? { ...prev.callCenter.agent, agent_status: nextStatus } : prev.callCenter.agent,
        },
      }));
    } catch (err) {
      alert(err.message || 'Failed to update call center status');
    }
    setCcSaving(false);
  }
  ...
  const statusColor = currentStatus === 'Available' || currentStatus === 'Available (On Demand)' ? 'bg-green-500' :
    ...
    currentStatus === 'Do Not Disturb' ? 'bg-red-500' :
        ...
        <div className="flex items-center gap-1" style={{ WebkitAppRegion: 'no-drag' }}>
          {ccEnabled && (
            <div className="flex items-center gap-1.5 mr-3 px-2 py-1 rounded-md bg-gray-50 border border-gray-200">
              ...
              <select
                value={currentStatus}
                disabled={ccSaving || statusOptions.length === 0}
                onChange={(e) => onCcStatusChange(e.target.value)}
